...
def tt_times_matrix_fwd(tensor, matrix, quantizer, return_saved_tensors):
    """
    ...
    for k in range(d):
        ...
        data = quantizer(torch.einsum('ria, rib->ba', [data, curr_core]))
    ...
    for k in range(d):
        ...
        data = quantizer(torch.einsum('ra, rjb->baj', [data, curr_core]))
...
def cp_times_matrix_fwd(tensor, matrix, quantizer, return_saved_tensors=False):
    """
    ...
    output = quantizer(torch.einsum('a...n,ar->...nr', output, tensor.factors[0]))
    saved_tensors.append(output)
    for factor in tensor.factors[1:order]:
        output = quantizer(torch.einsum('a...nr,ar->...nr', output, factor))
        saved_tensors.append(output)
    
    # forward propagate with output factors
    for factor in tensor.factors[order:tensor.order-1]:
        output = quantizer(torch.einsum('n...r,ar->n...ar', output, factor))
        saved_tensors.append(output)
    output = quantizer(torch.einsum('n...r,ar->n...a', output, tensor.factors[-1]))
